...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/connection/connect", async (req, res) => {
    ...
    try {
      ...
      // Para simulação, vamos gerar diretamente um QR Code em vez de chamar o webhook
      console.log(`Tentando conectar usando webhook: ${webhookUrl}`);
      
      // Podemos tentar chamar o webhook real em produção, mas para fins de teste,
      // vamos sempre gerar um QR code simulado
      connectionStatus[id] = {
        connected: false,
