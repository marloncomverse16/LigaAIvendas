...
/**
 * Controller para buscar números de telefone disponíveis na conta Meta
 */
export async function getMetaPhoneNumbers(req: Request, res: Response) {
  if (!req.isAuthenticated()) {
    return res.status(401).json({ message: 'Não autenticado' });
  }
  ...
  try {
    ...
    const apiVersion = settings.whatsappMetaApiVersion || 'v18.0';
    console.log(`Buscando números de telefone da Meta API. BusinessID: ${settings.whatsappMetaBusinessId}`);
    ...
    if (!phoneNumbersResponse.ok) {
      ...
      console.error('Erro ao buscar números de telefone:', errorData);
      return res.status(400).json({
        message: 'Não foi possível buscar números de telefone da Meta API',
        error: errorData.error?.message || 'Erro desconhecido'
      });
    }

    const phoneData = await phoneNumbersResponse.json();
    console.log('Números de telefone encontrados:', phoneData);
    
    if (!phoneData.data || phoneData.data.length === 0) {
      return res.status(404).json({
        message: 'Nenhum número de telefone encontrado na sua conta Meta Business'
      });
    }

    // Formatar dados dos números para o frontend
    const phoneNumbers = phoneData.data.map((phone: any) => ({
      id: phone.id,
      displayPhoneNumber: phone.display_phone_number,
      verifiedName: phone.verified_name,
      status: phone.status,
      qualityRating: phone.quality_rating,
      businessAccountId: settings.whatsappMetaBusinessId
    }));
