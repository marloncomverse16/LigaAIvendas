...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Rota para desconectar e excluir instância manualmente
  app.post("/api/connections/disconnect", async (req: Request, res: Response) => {
    ...
    try {
      ...
      console.log(`🗑️ Usuário solicitou desconexão manual da instância '${instanceName}'`);
      
      // Excluir instância da Evolution API
      try {
        const deleteResponse = await fetch(`${server.apiUrl}/instance/delete/${instanceName}`, {
          method: 'DELETE',
          headers
        });
        
        if (deleteResponse.ok) {
          console.log(`✅ Instância '${instanceName}' excluída manualmente com sucesso`);
          return res.status(200).json({
            success: true,
            message: "Instância desconectada e excluída com sucesso",
            instanceDeleted: true
          });
        } else {
          console.log(`⚠️ Resposta da exclusão: ${deleteResponse.status} - ${deleteResponse.statusText}`);
          return res.status(200).json({
            success: true,
            message: "Desconexão solicitada (instância pode não existir)",
        ...
        return res.status(200).json({
          success: true,
          message: "Desconexão solicitada (erro na comunicação com API)",
          instanceDeleted: false
        });
      }
    } catch (error: any) {
