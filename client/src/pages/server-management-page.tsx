...
export default function ServerManagementPage() {
  ...
  // Função para remover associação entre usuário e servidor
  const handleRemoveUserServer = (userId: number) => {
    if (!selectedServer) return;
    
    removeUserServerMutation.mutate({
      userId,
      serverId: selectedServer.id
    }, {
      onSuccess: () => {
    ...
    <div className="container mx-auto py-6">
      ...
      <Dialog open={userServerDialogOpen} onOpenChange={setUserServerDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>Usuários Conectados ao Servidor</DialogTitle>
            <DialogDescription>
              Gerencie os usuários que têm acesso ao servidor {selectedServer?.name}. 
              (Limite: {selectedServer?.maxUsers} usuários)
            </DialogDescription>
          </DialogHeader>
          
          <div className="space-y-4">
            {/* Lista de usuários já associados */}
            <div className="border rounded-lg p-3">
              <h3 className="text-sm font-medium mb-2">Usuários Conectados</h3>
              {isLoadingServerUsers ? (
                  ...
                  {serverUsers.map((serverUser: any) => (
                    <div key={serverUser.userId} className="flex items-center justify-between rounded-md border p-2">
                      <div className="flex items-center space-x-2">
                        ...
                        <div>
                          <p className="text-sm font-medium">{serverUser.user?.name || serverUser.user?.username}</p>
                          <p className="text-xs text-muted-foreground">{serverUser.user?.email}</p>
                        </div>
                      </div>
                      <Button 
                        variant="ghost" 
                        size="sm"
                        onClick={() => handleRemoveUserServer(serverUser.userId)}
                      >
                        <Trash2 className="h-4 w-4 text-red-500" />
                      </Button>
                    </div>
                ...
                <p className="text-sm text-muted-foreground py-2">Nenhum usuário associado a este servidor.</p>
              )}
            </div>
            
            {/* Adicionar novo usuário */}
            {!isServerUserLimitReached() && (
              <div className="border rounded-lg p-3">
                <h3 className="text-sm font-medium mb-2">Adicionar Usuário</h3>
                {isLoadingUsers ? (
                  ...
                  <div className="space-y-3">
                    <Select onValueChange={(value) => setSelectedUserId(Number(value))}>
                      ...
                      <SelectContent>
                        {users
                          .filter((u: any) => !serverUsers?.some((su: any) => su.userId === u.id))
                          .map((user: any) => (
                  ...
                  <p className="text-sm text-muted-foreground">Nenhum usuário disponível para associar.</p>
                )}
              </div>
            )}
            ...
            {isServerUserLimitReached() && (
              <div className="rounded-lg bg-yellow-50 border border-yellow-200 p-3">
                <p className="text-sm text-yellow-800">
                  <strong>Limite atingido:</strong> Este servidor já tem o número máximo de usuários ({selectedServer?.maxUsers}).
                  Para adicionar mais usuários, aumente o limite nas configurações do servidor.
                </p>
              </div>
            )}
          </div>
